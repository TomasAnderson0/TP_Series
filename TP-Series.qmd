...
La normalidad de los residuos no se cumple.
...
datos_total %>%  mutate(color = c(datos$agricultura* 0, rep(1,4)),
  ...
  scale_x_date(limits = c(as.Date("2004-01-01"),as.Date("2024-04-01")),
               breaks = seq(from = as.Date("2004-01-01"),
                            to = as.Date("2024-04-01"), by = "2 years"),
               date_labels = "%Y") + geom_point(color = "#D34343", size = 1) +
  geom_line(aes(y = prediccion, color = factor(color))) +
  scale_color_discrete(type = c("steelblue", "yellow"), 
                       labels = c("Observados", "Predichos"),
                       name = c("Valores")) +
  geom_point(aes(y = 33356.41, 
                 x = as.Date("2023-07-01")), 
             color = "black", size = 1) +
  geom_point(aes(y = 34192.26, 
                 x = as.Date("2023-10-01")), 
             color = "black", size = 1)+
  geom_point(aes(y =  37925.45, 
                 x = as.Date("2024-01-01")), 
             color = "black", size = 1)+
  geom_point(aes(y =  84060.91, 
